    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float _InvertDepth_var = lerp( node_8934, (1.0 - node_8934), _InvertDepth );
                float2 sceneUVs = float2(1,grabSign)*i.screenPos.xy*0.5+0.5 + (float3(node_7340.rg,0.0)*(_AffectDepth*_InvertDepth_var)*_Refraction*i.normalDir).rg;
...
///////// Gloss:
                ...
                float node_9000 = saturate(( _GlossSmooth > 0.5 ? (1.0-(1.0-2.0*(_GlossSmooth-0.5))*(1.0-(node_4795*_Gloss))) : (2.0*_GlossSmooth*(node_4795*_Gloss)) ));
                float gloss = node_9000;
...
/// Final Color:
                float3 finalColor = diffuse + specular;
                return fixed4(finalColor * saturate(((pow(node_4795,_OpacPowerRMultGAddBDepthW.r)*_OpacPowerRMultGAddBDepthW.g)+_OpacPowerRMultGAddBDepthW.b+node_8934)),0);
            }
            ENDCG
        }
    }
